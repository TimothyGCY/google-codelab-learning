...
import glab.bleckshiba.lemonade.DrawableMatcher.withDrawable
...
open class BaseTest {
     ...
     */
    fun drinkJuice() {
        onView(withDrawable(R.drawable.lemon_drink)).perform(click())
    }

    /**
     * Click the empty glass to restart
     */
    fun restart() {
        onView(withDrawable(R.drawable.lemon_restart)).perform(click())
    }

    /**
     * Extension function to determine if element is present.
     * This is used to click on the lemon image until it changes because of the number of clicks
...
object DrawableMatcher {
    ...
    private fun isSameBitmap(item: View, drawable: Drawable?, expectedResId: Int): Boolean {
        ...
        if (expectedResId < 0) return image.drawable == null;
